# 🔧 הוראות — Apps Script Setup ללקוח

## 📋 מה שצריך לעשות:

### שלב 1️⃣: הכנס את הקוד

1. **פתח את Apps Script** (כבר עשיתם ✅)
2. **בחר את כל הקוד בעמוד** (Ctrl+A)
3. **מחוק אותו**
4. **הדבק את הקוד מלמטה** ↓↓↓
...
## 💾 שלב 3️⃣: שמור את הקוד

1. **Ctrl+S** (Save)
2. Project name: **"ParkLog"** (או כל שם שתרוצה)

---

## 🚀 שלב 4️⃣: Run setupSheets()

1. בחלק העליון של Editor, לחץ על **dropdown** (שנרשום בו תפקידים)
2. בחר **setupSheets**
3. לחץ ▶️ **Run**
4. קבל הודעה: *"Sheets created/verified successfully!"*

אם ראית את ההודעה — **אתה בחינם!** ✅

---

## 📦 שלב 5️⃣: Deploy כ-Web App

### א. Deploy
1. בחלק העליון → **Deploy** → **New Deployment**
2. לחץ על ⚙️ (gear icon) בשמאל

### ב. הגדרות
```
Type: Web app
Execute as: [בחר את עצמך — name שלך]
Who has access: Anyone
```

### ג. Deploy
1. לחץ **Deploy**
2. אתה תקבל הודעה עם **URL**
...
### "Permission denied"
→ ודא שאתה בעל ה-Google Sheet (צריך להיות בחשבונך)

### "SHEET_ID is not defined"
→ ודא שהעתקת את כל הקוד (כולל השורה `const SHEET_ID = ...`)

### "Sheets created/verified" לא הופיע
→ בדוק את **Logs**: Tools → Logs
...
כשתסיים:
1. ✅ Sheets (Vehicles + Entries) יוצרו בשלך
2. ✅ Apps Script deployed
3. ✅ Web App URL זה שלך

**שלח לי את ה-URL ואנחנו סיימנו!** 🚀
